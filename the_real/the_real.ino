...
char topic_uscita[] = "out_arduino_rv";

//topic dal quale riceviamo messaggi
char receiving_topic[] = "in_arduino_rv";

//topic per il keep alive
char topic_alive[] = "alive";
...
void keepAlive(){
  String payload = "death_"; //messaggio che arriva su Edge quando arduino è offline
  ...
  pubblica(topic_alive,payload);     //pubblico il messaggio
}
...
void pubblica(char topic[], String payload){
  ...
  {
    Serial.print("Inviato al topic: ");
    Serial.println(topic);
  }
  else
  {
    Serial.print("NON riuscito al topic: ");
    Serial.println(topic);
    mqttClient.connect(id_client, "admin", "admin123");
  }
}
...
void sendMessage(){
  ...
  */
  ...
  else
  {
    ...
    pubblica(topic_uscita,payload);     //pubblico il messaggio
  }
}
